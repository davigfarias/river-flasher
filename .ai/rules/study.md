...
## is_starred + "estudar com estrela" personalized session
Cards get a manual `is_starred` flag (Card::scopeStarred), toggled only from the study screen itself (`toggleStarred()` in study.php, inline update like `toggleImageHidden()` — no dedicated action, star icon in the sticky header, color-coded amber via `text-amber-500` since Flux ships no filled/solid icon variant here). Not editable from deck-show or the card editor — marking happens mid-study when the user notices a card needs extra review.

`study.php` has `#[Url] public bool $starred = false`, threaded through `startSession()` -> `StartStudySessionOrchestrator::handle(..., onlyStarred: ...)` -> `FindCardsToStudy::handle(..., onlyStarred: ...)` which adds `->starred()` to the query. Kept in the URL like `mode` so `restart()` rebuilds the same personalized session.

`study-mode-modal.blade.php` splits only the Significado (Meaning) button into "Estudar" | "Estudar com estrela" — the second passes `starred: true` to `url()` (adds `?starred=1`) and is disabled (via computed `hasStarredCards`) when the deck/scope has zero starred active cards, same gating pattern as leitura/tradução. Reading and translation modes have no starred variant.
